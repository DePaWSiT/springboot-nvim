...
local function create_generate_class_ui()
  ...
  local main_class = utils.find_main_application_class_directory()
  local width = 60
  local height = 9

  local row = math.floor((vim.fn.winheight(0) - height) / 2)
  local col = math.floor((vim.fn.winwidth(0) - width) / 2)
  local popup_components =
    ui_utils.draw_popup(width, height, row, col, "Generate Class")
  ...
  local class_components = ui_utils.draw_labeled_input(
    46,
    1,
    row + 4,
    col + 1,
  ...
  api.nvim_buf_set_lines(popup_components.popup_buf, row + 8, -1, false, {
    edit_package .. string.rep(
      " ",
      60 - (string.len(edit_package) + string.len(edit_class))
    ) .. edit_class,
  })
  api.nvim_buf_set_lines(popup_components.popup_buf, row + 9, -1, false, {
    confirm_class .. string.rep(
      " ",
      60 - (string.len(confirm_class) + string.len(close_menu))
    ) .. close_menu,
  })
  api.nvim_set_current_win(components.wins.package_win)
...
local function create_generate_record_ui()
  ...
  local col = math.floor((vim.fn.winwidth(0) - width) / 2)
  local popup_components =
    ui_utils.draw_popup(width, height, row, col, "Generate Record")
  local package_components = ui_utils.draw_labeled_input(
    46,
    1,
    row + 1,
  ...
  local record_components = ui_utils.draw_labeled_input(
    46,
    1,
    row + 4,
    col + 1,
...
local function create_generate_interface_ui()
  ...
  local col = math.floor((vim.fn.winwidth(0) - width) / 2)
  local popup_components =
    ui_utils.draw_popup(width, height, row, col, "Generate Interface")
  local package_components = ui_utils.draw_labeled_input(
    46,
    1,
    row + 1,
  ...
  local interface_components = ui_utils.draw_labeled_input(
    46,
    1,
    row + 4,
    col + 1,
...
local function create_generate_enum_ui()
  ...
  local col = math.floor((vim.fn.winwidth(0) - width) / 2)
  local popup_components =
    ui_utils.draw_popup(width, height, row, col, "Generate Enum")
  local package_components = ui_utils.draw_labeled_input(
    46,
    1,
    row + 1,
  ...
  local enum_components = ui_utils.draw_labeled_input(
    46,
    1,
    row + 4,
    col + 1,
